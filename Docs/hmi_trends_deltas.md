...
- Raw path unified into a single SQL query (UNION ALL with `is_in_window` discriminant). Per-tag prev observability lines collapsed into `prev=N` count on the summary log. One connection per raw tile (was 2 via Promise.all).
- Cleanup: removed all `[xboundary-debug]` and `[zoom-debug]` diagnostic logs from client and server source. `TIMESCALE_LOG_TILE_QUERIES` set to 0 in `apps/caro-hmi/server/.env` (gate code preserved for future debugging).
