...
- Cleanup: `hadNullsAtFetch` removed from `CachedEntry` (Phase 3 invalidation never built); `rawEntries` removed from `AggregateTail` and `buildAggregateTail`; cross-bucketSMs re-bucket branch in `mergeAggregate` replaced with `return cached`; `TREND_RING_CAPACITY` reduced 100→20; §5.6 edge case deferred to acceptable one-frame live-overlay loss.
- Vestige removal: `Tooltip.tsx` deleted (no callers); `closeBucketsFromRing.ts` deleted (no internal callers after cross-bucketSMs removal); `commitAndDrain` returns `void` (range computation removed — only drain+clear needed); unmount cleanup useEffect deleted (evictAll on live-entry already clears cache); `handlePreset` routed through `dispatchModeAction`; `seedFromCachedTile` aggregate-only; `evictAll` JSDoc corrected; §10.6-§10.7 updated.
