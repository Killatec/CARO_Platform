...
1. `useTrendData` exposes `rangeExceeded: boolean` and `rangeTooNarrow: boolean` states. These flags serve **fetch suppression only** — they are keyed on `dataViewport`, which `useZoomState` does not update on every wheel tick when `lastIntent='zoom'`. The history-mode effect short-circuits before constructing tiles when `viewportSpan > MAX_VIEWPORT_SPAN_MS` (sets `rangeExceeded = true`) or `viewportSpan < MIN_VIEWPORT_SPAN_MS` (sets `rangeTooNarrow = true`), then returns. `ensureCovered` is gated by the same `isViewportOverRange` / `isViewportUnderRange` helpers so pan-extension fetches are also suppressed in both directions. `TrendChartContainer` derives its own modeViewport-keyed booleans for UX decisions; see point 3.
