...
6. The REST route in `apps/caro-hmi/server/src/routes/trends.ts` no longer derives or validates `bucketS`. `INVALID_BUCKET_S` is thrown by `getTrendTile()` in the bucketed branch for non-viewer consumers that bypass the client-side gate with an extreme over-range span.
