...
| `INVALID_RANGE_TOO_NARROW` | `end_time − start_time < MIN_VIEWPORT_SPAN_MS` (1000 ms). Defense-in-depth guard for non-viewer consumers that bypass the client-side `gatedFetchTile` gate. Normal viewer clients never trigger this because `gatedFetchTile` rejects with `CLIENT_UNDER_RANGE` before issuing any fetch (§6.3 Out-of-range UX). |
...
1. `useTrendData` exposes `rangeExceeded: boolean` and `rangeTooNarrow: boolean` states. The history-mode effect short-circuits before constructing tiles when `viewportSpan > MAX_VIEWPORT_SPAN_MS` (sets `rangeExceeded = true`) or `viewportSpan < MIN_VIEWPORT_SPAN_MS` (sets `rangeTooNarrow = true`), then returns. `ensureCovered` is gated by the same `isViewportOverRange` / `isViewportUnderRange` helpers so pan-extension fetches are also suppressed in both directions.
2. All four fetch sites in `useTrendData` (live-spine, history-visible, history-prefetch, `ensureCovered`) delegate through a single `gatedFetchTile` `useCallback` wrapper that rejects with `CLIENT_PRE_EPOCH` for pre-epoch tiles (`startTime < 0n`), `CLIENT_UNDER_RANGE` when `tileSpanMs / bucketCount === 0n` (sets `rangeTooNarrow = true`), and `CLIENT_OVER_RANGE` when `bucketS > MAX_BUCKET_S` (sets `rangeExceeded = true`). Catch handlers at each site recognize these sentinels as silent no-ops; `CLIENT_OVER_RANGE` sets `batchHasRangeExceeded` and `CLIENT_UNDER_RANGE` sets `batchHasRangeTooNarrow` in `historyTileFetch` to prevent `performSwap` from clearing the flags mid-batch.
3. `TrendChartContainer`'s render branch uses `chartData = (rangeExceeded || rangeTooNarrow) ? placeholderData : mergedData`. `placeholderData` carries `n = 2`, `bucketSMs = Number(span)`, and one null-filled series entry per `tagId`. The two stub x-values at `modeViewport.start` and `modeViewport.end` let uPlot autoscale the x-axis to the user's selected range (preventing the `[0,1]` fallback that would otherwise produce negative-timestamp feedback on subsequent wheel events).
...
6. The REST route in `apps/caro-hmi/server/src/routes/trends.ts` applies two server-side defense-in-depth checks: `INVALID_BUCKET_S` (thrown by `getTrendTile()` in the bucketed branch for extreme over-range spans) and `INVALID_RANGE_TOO_NARROW` (route-level 400 when `end_time − start_time < MIN_VIEWPORT_SPAN_MS`). Normal viewer clients never reach either — the client-side `gatedFetchTile` gate rejects before issuing any fetch.
...
- **`CLIENT_UNDER_RANGE` sentinel in `gatedFetchTile` + `rangeTooNarrow` state + `INVALID_RANGE_TOO_NARROW` on the route.** The primary under-range guard: `gatedFetchTile` checks `tileSpanMs / BigInt(bucketCount) === 0n` and, if true, sets `rangeTooNarrow = true` and rejects with `CLIENT_UNDER_RANGE` without calling the server. The history-mode effect has a matching `isViewportUnderRange` short-circuit (`viewportSpan < MIN_VIEWPORT_SPAN_MS`) and `ensureCovered` applies the same guard — together these suppress all tile fetches when the viewport is too narrow. The REST route adds `INVALID_RANGE_TOO_NARROW` (400) as defense in depth for consumers that bypass the client gate. Parallel to the `CLIENT_OVER_RANGE` / `INVALID_BUCKET_S` / `rangeExceeded` pattern (see §6.3 Out-of-range UX).
