...
`n` is the **actual** row count in each `value` array. For aligned requests using CAG sources (bucket widths 1s/10s/1min/10min that divide cleanly into `TS_BUCKET_ORIGIN_MS`), `n === bucket_count`. For unaligned CAG-source requests, `time_bucket_gapfill` emits one extra leading bucket, so `n === bucket_count + 1`. For the raw-source bucketed path (`source === 'tag_samples'`, sub-second bucket widths derived from `Math.round(spanMs / bucket_count)`), `n` can range up to `bucket_count + 3`. When `Math.round` rounds down, `bucketSMs < spanMs/bucket_count`, so `q = spanMs/bucketSMs > bucket_count` and `floor(q)` can reach `bucket_count + 1`; combined with the `+2` gapfill endpoint variance, the maximum is `bucket_count + 3`. Clients must use the response's actual `n` field and `bucketSMs`; never assume the bucket count matches the request.
