...
`moduleTs` is the **source-message** timestamp, not the HMI server's receipt time. Every tag in a single enqueue shares the same `moduleTs`. The flush timer runs at `DB_TICK_MS` (default 1000 ms) in `index.ts`:
...
| `DB_TICK_MS`              | `1000`                 | `index.ts`            | DB-pipeline flush interval. Placeholder today. |
| `WATCHDOG_TIMEOUT_MS`     | `5000`                 | `telemetry-intake.ts` | Time without telemetry before nulling the module's LKV entries. |
| `HEARTBEAT_INTERVAL_MS`   | `3000`                 | `mqtt-bridge.ts`      | Heartbeat publish frequency on `caro/{module_id}/beat`. |
