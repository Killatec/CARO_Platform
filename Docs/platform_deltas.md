...
- @caro/ui Modal: added `outerStyle`, `headerStyle`, `bodyStyle` inline-style props (CSSProperties) as escape hatches for inline-style consumers. Existing className props retained for Tailwind-native consumers (BooleanSet, tag-registry modals). No back-compat break.
