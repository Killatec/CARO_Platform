...
| `@caro/trend-chart` | `packages/trend-chart/` | Trend chart UI package. Geometry + cache: `level.ts` (alignedTilesInRange, tilesForViewport — bucketCount=500/visibleTilesPerWindow=2/overfetchPerSide=1), `tileCache.ts` (LRU keyed by tagId+startTime+endTime+bucketCount, 50 MB cap), `colorAssign.ts` (schemeTableau10 ×2). React hook: `useTrendData` (2-tile visible + 2 prefetch parallelism, ⌈N/8⌉ fan-out, stale-generation guard). Components: `TrendChart` (uPlot canvas wrapper, stepped interpolation, per-trace Y-scales), `Legend`, `Tooltip` (site-timezone aware), `ResolutionIndicator`. Consumed by `apps/caro-hmi/client`. |
...
**Next priority — Step 10: Mode state machine + time-range UI.** Steps 1–9 are complete: Trends API (v0.7), watermark fall-through, `@caro/trend-chart` package (cache/geometry, `useTrendData`, `TrendChart` static rendering). Step 10 wraps `TrendChart` in a stateful container covering tailing/fixed mode transitions (§9.3), preset time strip (§12.1), custom range picker (§12.2), Live button (§12.3), and pan/zoom interactions (§9.1–9.2). First production wiring of `useTrendData` — still no WS. Build order and scope in `Docs/hmi_trend_viewer_spec.md` §17.1.1 steps 10–12. Full TODO list in `Docs/platform_todo.md`.
