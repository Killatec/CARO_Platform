...
| CARO HMI | 3003 | 5175 | Phase A Steps 1–11 complete. Server core + client shell + demo pages + full trends read + live tail. HmiTagSource publishes 7 Trend_Info observability tags every 250ms. TrendSnapshotScheduler guarantees ≥1 DB row/tag/minute. **Trends API v0.9:** `GET /api/v1/trends/tile` — dispatches on `bucketS` across raw + four CAGs; watermark-aware fall-through; aggregate response includes `bucketSMs`, `responseTailTs`, per-series `min`/`max`; three-case post-pass; N≤8 cap; optional gzip. **Trend WS channel (Step 11):** SUBSCRIBE_TREND/UNSUBSCRIBE_TREND/TREND_DELTA messages on shared WS connection; per-client outbox; TREND_FLUSH_HZ=4 Hz flush cadence; synthetic-on-flush events; `setTrendDeltaListener` wires TelemetryIntake to WsServer outbox. **`@caro/trend-chart` package (Step 11):** `useLiveSubscription` (FIFO + bucket accumulator for aggregate; raw buffer trimmed to 2×viewportSpanMs; `commitAndDrain`), `mergeTrendData` (isTailing live-wins overlap), `isTailing` skip guard in `useTrendData`, `dispatchModeAction` tailing-exit cleanup. LOCF cutoff removed (814ms planning regression eliminated); watermark memoized (30s TTL); raw path unified (UNION ALL). Test coverage: 542 @caro/trend-chart, 67 @caro/hmi-context, 109 @caro/db, 236 server, 33 client. Next: Step 12 — Tag picker drawer. |
...
| `@caro/trend-chart` | `packages/trend-chart/` | Trend chart UI package. Geometry + cache: `level.ts` (alignedTilesInRange, tilesForViewport — tile-grid right-anchor, look-ahead prefetch filter; bucketCount=500/visibleTilesPerWindow=2/overfetchPerSide=1; `bucketSMs === 0n` guard), `tileCache.ts` (LRU keyed by tagId+startTime+endTime+bucketCount, 50 MB cap), `colorAssign.ts` (schemeTableau10 ×2). Hooks: `useTrendData` (2-tile visible + 2 prefetch parallelism, ⌈N/8⌉ fan-out, stale-generation guard; `isTailing` skip guard suppresses fetches during tailing + unchanged span + non-empty active set; `responseTailTs` field), `useTrendMode` (tailing/fixed mode reducer + hook; `liveClicked` is sole fixed→tailing path; `tick` dispatched by container on TREND_DELTA receipt), `useZoomState` (zoom level state + `computeDragZoomViewport`), `useLiveSubscription` (Step 11: SUBSCRIBE_TREND/UNSUBSCRIBE_TREND lifecycle; FIFO per tag + bucket accumulator for aggregate mode; raw buffer per tag trimmed to 2×viewportSpanMs; `commitAndDrain` for atomic tailing exit; boolean coercion in `toNumericValue`; TREND_FIFO_CAPACITY), `mergeTrendData` (Step 11: merges cached data + LiveTail; in tailing mode live wins overlap — aggregate clips to liveEndIndex, raw drops ts≥minLiveTs entries). Render: `uplotConfig.ts` (always-band 2-series shape; bucketSMsKey dep for coverage-check effect), `bandsFromTrendData.ts`. Components: `TrendChartContainer` (stateful wiring; `dispatchModeAction` wrapper for atomic tailing-exit cleanup; stable refs updated during render), `TrendChart`, `SpanPresets`, `EndPicker`, `SpanBucketIndicator`, `Legend`. `axisInteractions.ts` exports 9 pure pan/zoom helpers. Consumed by `apps/caro-hmi/client`. See `Docs/hmi_trend_viewer_handoff.md` for full file map. |
...
**Next priority — Step 12: Tag picker.** Steps 1–11 are complete. Step 11 (live tail) shipped 2026-05-11: dedicated trend WS channel (SUBSCRIBE_TREND/UNSUBSCRIBE_TREND/TREND_DELTA), `useLiveSubscription` hook (FIFO + bucket accumulator for aggregate, raw buffer for raw mode, 2×viewportSpanMs trim, `commitAndDrain`), `mergeTrendData` (live-wins overlap, isTailing clip), `isTailing` tile-fetch suppression, `dispatchModeAction` tailing-exit cleanup. 542 `@caro/trend-chart` + 67 `@caro/hmi-context` tests. Step 12 wires the tag picker drawer (tree + search + multi-select commit + trendable filter). See `Docs/hmi_trend_viewer_handoff.md` for full subsystem detail. Full TODO list in `Docs/platform_todo.md`.
