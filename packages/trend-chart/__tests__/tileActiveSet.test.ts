...
function makeEntry(startMs: bigint, endMs: bigint, bucketCount = 500): ActiveTileEntry {
  return { tile: { startTime: startMs, endTime: endMs, bucketCount }, responseTailTs: null, shape: null, data: null };
}
...
describe('makeActiveTileEntryFromCache', () => {
  ...
  it('source=raw → shape=raw, responseTailTs from cached entry', () => {
    const cached: CachedEntry = { source: 'raw', responseTailTs: TAIL_TS, ts: [], valueRaw: [] };
  ...
  for (const source of aggSources) {
    it(`source=${source} → shape=aggregate`, () => {
      const cached: CachedEntry = { source, responseTailTs: TAIL_TS, value: [], bucketSMs: 3_600, n: 500 };
      const entry = makeActiveTileEntryFromCache(tile, cached);
      expect(entry.shape).toBe('aggregate');
      expect(entry.responseTailTs).toBe(TAIL_TS);
    });
  ...
  it('missing responseTailTs in cached entry → null', () => {
    const cached: CachedEntry = { source: '1min_cagg', value: [] };
    const entry = makeActiveTileEntryFromCache(tile, cached);
    expect(entry.responseTailTs).toBeNull();
  });
});

// ─── computeResponseTailTs ────────────────────────────────────────────────────

describe('computeResponseTailTs', () => {
  it('all entries null → returns null', () => {
    const entries: ActiveTileEntry[] = [
      makeEntry(0n, HALF_HOUR),
      makeEntry(HALF_HOUR, HALF_HOUR * 2n),
    ];
    expect(computeResponseTailTs(entries)).toBeNull();
  });

  it('single non-null entry → returns its value', () => {
    const entry = { ...makeEntry(0n, HALF_HOUR), responseTailTs: 1_700_000_000_000 };
    expect(computeResponseTailTs([entry])).toBe(1_700_000_000_000);
  });

  it('returns max across entries with non-null responseTailTs', () => {
    const entries: ActiveTileEntry[] = [
      { ...makeEntry(0n, HALF_HOUR),               responseTailTs: 1_000 },
      { ...makeEntry(HALF_HOUR, HALF_HOUR * 2n),   responseTailTs: 9_000 },
      { ...makeEntry(HALF_HOUR * 2n, HALF_HOUR * 3n), responseTailTs: 5_000 },
    ];
    expect(computeResponseTailTs(entries)).toBe(9_000);
  });

  it('ignores entries with null responseTailTs in max computation', () => {
    const entries: ActiveTileEntry[] = [
      { ...makeEntry(0n, HALF_HOUR),             responseTailTs: null },
      { ...makeEntry(HALF_HOUR, HALF_HOUR * 2n), responseTailTs: 7_000 },
      { ...makeEntry(HALF_HOUR * 2n, HALF_HOUR * 3n), responseTailTs: null },
    ];
    expect(computeResponseTailTs(entries)).toBe(7_000);
  });

  it('empty array → returns null', () => {
    expect(computeResponseTailTs([])).toBeNull();
  });
});
